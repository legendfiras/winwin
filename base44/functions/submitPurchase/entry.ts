import { createClientFromRequest } from 'npm:@base44/sdk@0.8.31';
import {
  readBody, json, db, requireCustomerSession, pointsForPurchaseUsd, enrichCustomer,
} from '../_shared/helpers.ts';

Deno.serve(async (req) => {
  try {
    const base44 = createClientFromRequest(req);
    const database = db(base44);
    const body = await readBody(req);
    const authz = await requireCustomerSession(database, body.session_token);
    if (authz.error) return authz.error;

    const amount_usd = Number(body.amount_usd);
    if (!Number.isFinite(amount_usd) || amount_usd <= 0) {
      return json({ error: 'Enter a valid purchase amount' }, 400);
    }
    const calculated_points = pointsForPurchaseUsd(amount_usd);
    const product_ids = Array.isArray(body.product_ids) ? body.product_ids : [];
    const product_summary = String(body.product_summary || '').trim();

    const tx = await database.entities.StoreTransaction.create({
      customer_id: authz.customer.id,
      customer_email: authz.customer.email,
      customer_name: authz.customer.full_name,
      type: 'PRODUCT_PURCHASE',
      status: 'PENDING',
      amount_usd,
      product_ids: JSON.stringify(product_ids),
      product_summary,
      calculated_points,
      submitted_by: 'CUSTOMER',
    });

    return json({
      success: true,
      transaction: tx,
      customer: await enrichCustomer(database, authz.customer),
    });
  } catch (error) {
    return json({ error: error.message }, 500);
  }
});
